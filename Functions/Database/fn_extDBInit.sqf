...
if (isNil {
    uiNamespace getVariable "extDB_SQL_CUSTOM_ID"
}) then {
    private _result = "extDB3"
    callExtension "9:VERSION";
    diag_log format["extDB3: Version: %1", _result];
    if (_result isEqualTo "") exitWith {
        diag_log "extDB3: Failed to Load Extension";
        _return = false
    };
    if ((parseNumber _result) < 1.026) exitWith {
        diag_log "Error: extDB3 version 1.026 or Higher Required";
        _return = false
    };
    _result = parseSimpleArray("extDB3"
    callExtension format["9:ADD_DATABASE:%1", _database]);
    if ((_result select 0) isEqualTo 0) exitWith {
        diag_log format["extDB3: Error Failed to Connect to Database: %1", _result];
        _return = false
    };
    ...
    _result = parseSimpleArray("extDB3"
    callExtension format["9:ADD_DATABASE_PROTOCOL:%1:%2:%3:%4", _database, _protocol, _extDB_ID, _custom]);
    if ((_result select 0) isEqualTo 0) exitWith {
