...
if (hasInterface) then {
    ...
    forEach["LARs_fnc_overrideVAButtonDown", "LARs_fnc_overrideVATemplateOK", "LARs_fnc_loadInventory_whiteList", "LARs_fnc_applyLBColors", "LARs_fnc_showRestrictedItems", "LARs_fnc_addVAKeyEvents"];
    ...
    [missionNamespace, "arsenalOpened", {
        ...
        DEBUG(_msg);
        _virtualItemCargo = (missionNamespace call BIS_fnc_getVirtualItemCargo) + (_cargo call BIS_fnc_getVirtualItemCargo) + items _center + assignedItems _center + primaryWeaponItems _center + secondaryWeaponItems _center + handgunItems _center + [uniform _center, vest _center, headgear _center, goggles _center];
        _virtualWeaponCargo = []; {
            if !(isNil _x || {
                _x == "%ALL" || _x == ""
            }) then {
        ...
        forEach[_virtualItemCargo, _virtualWeaponCargo, _virtualMagazineCargo, _virtualBackpackCargo];
