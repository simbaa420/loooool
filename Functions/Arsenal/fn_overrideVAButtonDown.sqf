    ...
    case (_key == DIK_TAB):
        {
            _idc = -1; {
                ...
                if !(ctrlEnabled _ctrlTab) exitWith {
                    _idc = _x;
                };
            }
            forEach[IDCS_LEFT];
            _idcCount = {
                !isNull(_display displayCtrl(IDC_RSCDISPLAYARSENAL_TAB + _x))
            }
            count[IDCS_LEFT];
            _idc =
            if (_ctrl) then {
                (_idc - 1 + _idcCount) % _idcCount
            } else {
                (_idc + 1) % _idcCount
            };
            if (BIS_fnc_arsenal_type == 0) then {
    ...
    case (_key == DIK_C):
        {
            ...
            if (_shift) then {
                ...
                if (_ctrl) then {
                    ['buttonExport', [_display, _mode]] call BIS_fnc_arsenal;
                };
            } else {
                if (_ctrl) then {
                    ['buttonExport', [_display, _mode]] call BIS_fnc_garage;
                };
            };
        };
    case (_key == DIK_V):
        {
            if (BIS_fnc_arsenal_type == 0) then {
                if (_ctrl) then {
                    ['buttonImport', [_display]] call BIS_fnc_arsenal;
                };
            } else {
                if (_ctrl) then {
                    ['buttonImport', [_display]] call BIS_fnc_garage;
                };
            };
        };
    case (_key == DIK_S):
        {
            if (_ctrl) then {
                ['buttonSave', [_display]] call BIS_fnc_arsenal;
            };
        };
    case (_key == DIK_O):
        {
            if (_ctrl) then {
                ['buttonLoad', [_display]] call bis_fnc_arsenal;
                [_display] call LARs_fnc_applyLBColors;
            };
        };
            ...
            if (_ctrl) then {
                if (BIS_fnc_arsenal_type == 0) then {
                    if (_shift) then {
                        ...
                        forEach("isclass _x && getnumber (_x >> 'scope') > 1 && gettext (_x >> 'simulation') == 'soldier'"
